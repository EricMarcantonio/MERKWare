...
def mix_single_column(a):
    ...
    t = a[0] ^ a[1] ^ a[2] ^ a[3]
    u = a[0]
    a[0] ^= t ^ fix_byte_80(a[0] ^ a[1])
    a[1] ^= t ^ fix_byte_80(a[1] ^ a[2])
    a[2] ^= t ^ fix_byte_80(a[2] ^ a[3])
    a[3] ^= t ^ fix_byte_80(a[3] ^ u)
...
def inv_mix_columns(s):
    ...
    for i in range(4):
        u = fix_byte_80(fix_byte_80(s[i][0] ^ s[i][2]))
        v = fix_byte_80(fix_byte_80(s[i][1] ^ s[i][3]))
        s[i][0] ^= u
        s[i][1] ^= v
        s[i][2] ^= u
        s[i][3] ^= v
